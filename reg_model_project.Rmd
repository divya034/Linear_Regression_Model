...
In conclusion, we may actually predict the popularity of a movie (as represented by the *audience_score* variable) by knowing only its genre, number of IMDb votes and Rotten Tomatoes critics score. The other variables we were interested in were not significantly related to our response. This assertion comes from the fact that we were able to build a linear model to predict our response with the three aforementioned statistically significant predictors. However, we must keep in mind that the adjusted R squared of our final model is `r summary(lm_movies)$adj.r.squared`, so this means that roughly `r summary(lm_movies)$adj.r.squared * 100`% of the varaiability is explained by the model, which is good, but not great. This fact may partly explain why we have a non negligible discrepancy between the audience score predicted by our model and the actual audience score of the "Captain America : Civil War" movie.
